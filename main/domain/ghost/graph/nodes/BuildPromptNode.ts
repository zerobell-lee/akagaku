...
export const BuildPromptNode = new RunnableLambda<GhostState, Partial<GhostState>>({
    func: async (state: GhostState) => {
        const { llmService } = state.llmProperties;
    ...
    ⚠️ **CRITICAL RESPONSE FORMAT RULE** ⚠️
    
    - Your response must be in **raw JSON format string**, without any comments or code blocks.
    - DON'T ADD YOUR EXPLANATION OR COMMENT IN YOUR RESPONSE. THE AGENT WILL FAIL TO PARSE YOUR RESPONSE IF YOU DO.
...
- Adjust tone based on affection and recent interactions (e.g., warmer if affection is high).
    ...
    If you fail to follow the guidelines, you'll be blamed by the system because of wasting time and memory.
            ...
            if (llmService === 'anthropic') {
                ...
                return openAIprompt2
            }
            return defaultPrompt
        }

        const buildPromptForTool = () => {
            return `
            You are an AI assistant that can call tools, and you're also capale of making a decision about calling tools.
        ...
        const result = {"tool_call_chain": [{"name": "functions.get_weather", "args": {"location": "Suwon"}, "result": "Partly cloudy with a temperature of 16째C."}]}

        return { promptForCharacter: buildPromptText(), promptForTool: buildPromptForTool() };
    }
});
